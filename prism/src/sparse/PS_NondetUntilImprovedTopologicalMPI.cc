...
)
{
	...
	int low, hi, M0, M1, M2, M3, K0, K1, K2, K3;
	int j, m, l1, h1, l2, h2, k, ind, dest, left, adv_temp, localitr;
	double d2, d1;
	bool min = false;
	...
	int* useful_states2 = new int[n];
	int* uf_choice_strt2 = new int[n];
	int* uf_cols2 = new int[choice_starts[row_starts[n] - 1] + 5];
	double* uf_nnz2 = new double[choice_starts[row_starts[n] - 1] + 5];
	...
	for(ind = 0; ind < number_of_sccs; ind++)
	{
		...
		// do matrix multiplication and min/max
			...
			while(M1 < M0)
			{
				...
				{
					in_probs[cols[j]] -= non_zeros[j];
					if(in_probs[cols[j]] <= 0 && scc_4[cols[j]] == ind)
						queue[M0++] = cols[j];
				}
			}
			K3 = M0 - 1;
			...
			for(m = low; m < hi; m++)
				...
				for(j = 0; j < choice_starts[1 + adv_starts[i]] - choice_starts[adv_starts[i]]; j++)
				{
					K2 = choice_starts[adv_starts[i]];
					uf_cols2[K1 + j] = cols[K2 + j];
					uf_nnz2[K1 + j] = non_zeros[K2 + j];
				}
				K0++;
				K1 += j;
				uf_choice_strt2[K0] = K1;
			...
			while(!terminate && localitr < mx_itr && hi - low > 1)
			{
				...
				{	
					...
					d1 = d2 = 0;
					l2 = uf_choice_strt2[m];
					h2 = uf_choice_strt2[m + 1];
					for(k = l2; k < h2; k++)
						d1 += uf_nnz2[k] * soln[uf_cols2[k]];					
