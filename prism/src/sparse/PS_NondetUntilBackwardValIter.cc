...
JNIEXPORT jlong __jlongpointer JNICALL Java_sparse_PrismSparse_PS_1NondetUntilBackwardValIter
(
JNIEnv *env,
jclass cls,
...
//	soln2 = new double[n];		Canceled for Gauss-Seidel.
	...
	int *pre_start = new int[n+1];
	int *pre_end = new int[n+1];
	int *pre_freq = new int[n+1];
 	int *pre_state = new int[choice_starts[row_starts[n]-1]+500];
	int *state_order = new int[n+1];
	...
	for(i = 0; i < n; i++)
		for(j = row_starts[i]; j < row_starts[i+1]; j++)
			for(k = choice_starts[j]; k < choice_starts[j+1]; k++)
				if((non_zeros[k]) >= .5/(choice_starts[j+1] - choice_starts[j]))	
					pre_freq[cols[k]]++;
	...
	for(int k = 0; k < n; k++)
	{
		if(yes_vec[k] < 1 && selected[k] == false && row_starts[k+1] > row_starts[k])
		{
			selected[k] = true;
			state_order[top++] = k;
		}
