...
JNIEXPORT jlong __jlongpointer JNICALL Java_sparse_PrismSparse_PS_1BUCKETSOR
(
JNIEnv *env,
jclass cls,
...
)
{
	...
	for(i = 0; i < n; i++)
	{
		...
		{
			if(fabs(non_zeros[j]) > p1)
			{
				p2 = p1;
				p1 = fabs(non_zeros[j]);
			}
			else if(fabs(non_zeros[j]) > p2)
					p2 = non_zeros[j];
							
		}
		for(j = row_starts[i]; j < row_starts[i+1]; j++)
		{
			if(fabs(non_zeros[j]) == p1 || fabs(non_zeros[j]) == p2)
				pre_freq[cols[j]]++;
		}		
	}
	pre_start[0] = 0;
