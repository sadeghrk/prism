...
{	
	...
	int num_actions;
	// misc
	int i, j, k, l, l1, h1, l2, h2, iters, localitr, left;
	double d1, d2, x, sup_norm, kb, kbt;
	bool done, first;
	double self; 
	double total_mults;
	int one_iter_mults, good_mults;
	...
	int *inp_dns_cols = new int[dtmc_max_trans];
	double *inp_dns_nnz = new double[dtmc_max_trans];
	double *in_probs = new double[n];
	double inp_sum, good_inp_sum;
	...
	{	 
		...
		for(i = 0; i < n; i++)
		{
			...
			if(in_probs[i] > .5)
			{
				...
				for(k = l2; k < h2; k++)
				{
					inp_dns_cols[left] = cols[k];
					inp_dns_nnz[left++] = non_zeros[k];
				}
			}
		}
		good_mults = left;
		...
		while(!done)
		{
			...
			else
			{
				...
				{	
					...
					for(k = l2; k < h2; k++)
						if(inp_dns_cols[k] != i)
							d1 += inp_dns_nnz[k] * soln[inp_dns_cols[k]];
						else
							self -= inp_dns_nnz[k];
					if(self > 0)
						d1 /= self;
