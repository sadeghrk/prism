			...
			try {
				...
				case Prism.SPARSE:
					...
					if(doTopologicalVI)
					{
						...
						if(prism.getMDPSolnMethod() == prism.MDP_EXPMODPOLITER)
						{
					...
					if (doIntervalIteration) {
						...
						if (transform != null) {
							...
							if(prism.getMDPSolnMethod() == prism.MDP_EXPMODPOLITER) {
							probsDV = PrismSparse.NondetUntilExplicitModPI(tr, tra, model.getSynchs(), odd, allDDRowVars, allDDColVars, allDDNondetVars, yes, maybe, min, strat);}
							else
							if(prism.getMDPSolnMethod() == prism.MDP_IMPROVEDMODPOLITER2) {
